...
//        	Log.printLine(i-id);
	...
	private static List<PowerHost> createPowerHostList(int id, int num_hosts, List<List<Pe>> pe2D, List<Integer> rams, List<Integer> bws, List<Long> storages, PowerModel powerModel) 
	{
...
//        	Log.printLine(i-id);
	...
	private static List<Rack> createPowerRackList(int rackid, int num_racks, List<List<PowerHost>> host2D) {
        List<Rack> rackList = new ArrayList<Rack>();
        for(int i = 0; i<num_racks; i++)
        {
            rackList.add(new Rack(rackid+i, host2D.get(i)));
...
//        double costPerBw = 0.0;			// the cost of using bw in this resource
	...
	private static PowerDatacenter createPowerDatacenter(double cost, double costPerMem, double costPerStorage, double costPerBw, List<Zone> zoneList)
	{
		// 5. Create a DatacenterCharacteristics object that stores the
        //    properties of a data center: architecture, OS, list of
        //    Machines, allocation policy: time- or space-shared, time zone
...
//        double costPerBw = 0.0;			// the cost of using bw in this resource
        ...
        try {
            datacenter = new PowerDatacenter("Datacenter", characteristics, new VmAllocationPolicySimple(characteristics.getHostList()), storageList, 0);
        } catch (Exception e) {
            e.printStackTrace();
        }
...
//				Log.printLine("zoneid: "+zoneid);
	...
	public PowerDatacenter create_powerinfrastructure(String str,String str1)
	{
		//Reader read = new Reader(str);	
		ReadConf read = new ReadConf(str);
		List<PowerDatacenter> dcList = new ArrayList<PowerDatacenter>();
		int datacentercount = read.getDatacenterCount();
...
//				System.out.println("j"+ j);
				...
				{
					List<Rack> racklist = new ArrayList<Rack>();
					List<List<PowerHost>> host2D = new ArrayList<List<PowerHost>>();
					int nr = rackid;
...
//							Log.printLine("hostid: "+hostid);
						...
						hostlist = createPowerHostList(n, read.getHostList().get(rackid), pe2D, rams, bws, storages, powerModel);
//						Log.printLine(hostlist);
						host2D.add(hostlist);
//						Log.printLine("rackid: "+rackid);
						rackid++;
...
//					racklist = createRackList(nr, read.getRackList().get(k+j*i), host2D);
					racklist = createPowerRackList(nr, read.getRackList().get(aisleid), host2D);
					rack2D.add(racklist);
//					Log.printLine(racklist);
//					Log.printLine("aisleid: "+aisleid);
					aisleid++;
...
//				Log.printLine("zoneid: "+zoneid);
			...
			zoneList = createZoneList(nz, read.getZoneCount(), aisle2D);
			dcList.add(createPowerDatacenter(3.0, 0.05, 0.001, 0.0, zoneList));
		}
